  ...
  return (
    <div className="space-y-12">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
          ...
          <h1 className="font-display text-[clamp(32px,5vw,48px)] leading-[1.1] text-[#0A0A0A] mb-4">
            Gestion du <span className="text-[#888888]">Parcours.</span>
          </h1>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2 p-1 bg-[#F7F7F5] border border-[#E8E8E4] rounded-2xl w-fit">
          <button 
            ...
            className={cn(
              ...
              activeTab === "experience" ? "bg-white text-[#0A0A0A] shadow-sm" : "text-[#888888] hover:text-[#0A0A0A]"
            )}
          >
          ...
          <button 
            ...
            className={cn(
              "px-6 py-3 rounded-xl font-body text-[12px] font-bold uppercase tracking-widest transition-all flex items-center gap-2",
              activeTab === "education" ? "bg-white text-[#0A0A0A] shadow-sm" : "text-[#888888] hover:text-[#0A0A0A]"
            )}
          >
        ...
        {/* Content Table */}
        <div className="bg-white rounded-3xl border border-[#E8E8E4] overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.02)]">
          {/* Filters bar */}
          <div className="p-6 md:p-8 bg-[#F7F7F5]/50 border-b border-[#E8E8E4] flex flex-col md:flex-row gap-6 items-center justify-between">
            ...
            <div className="flex items-center gap-2 font-body text-[12px] font-bold text-[#888888] uppercase tracking-widest">
              <Filter size={14} /> <span className="text-[#0A0A0A]">{filteredItems.length}</span> Entrées
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#E8E8E4]">
                  <th className="py-5 px-8 font-body text-[11px] font-bold text-[#888888] uppercase tracking-widest">Détails</th>
                  <th className="py-5 px-8 font-body text-[11px] font-bold text-[#888888] uppercase tracking-widest hidden md:table-cell">Dates</th>
                  <th className="py-5 px-8 font-body text-[11px] font-bold text-[#888888] uppercase tracking-widest">Status</th>
                ...
                {loading ? (
                  Array(3).fill(0).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td colSpan={4} className="px-8 py-10 h-24 bg-[#F7F7F5]/30"></td>
                    </tr>
                  ))
                ) : filteredItems.length > 0 ? (
                  filteredItems.map((item) => (
                    <tr key={item.id} className="group hover:bg-[#F7F7F5] transition-colors">
                      <td className="py-6 px-8">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-2xl bg-[#F7F7F5] border border-[#E8E8E4] flex items-center justify-center shrink-0 text-[#0A0A0A] group-hover:bg-white transition-all">
                             {activeTab === "experience" ? <Briefcase size={20} strokeWidth={1.5} /> : <GraduationCap size={20} strokeWidth={1.5} />}
                          </div>
                          <div>
                            <div className="font-body text-[15px] font-bold text-[#0A0A0A]">
                              {activeTab === "experience" ? item.role : item.degree}
                            </div>
                            <div className="font-body text-[12px] text-[#888888] uppercase tracking-widest mt-0.5">
                      ...
                      <td className="py-6 px-8 hidden md:table-cell">
                        <div className="flex flex-col gap-1">
                           <div className="flex items-center gap-2 font-mono text-[11px] font-bold text-[#888888]">
                              <Calendar size={12} />
                      ...
                      <td className="py-6 px-8">
                        {item.current ? (
                           <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-black text-white rounded-md text-[10px] font-bold uppercase tracking-widest">
                             <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" />
                             En cours
                           </div>
                        ) : (
                          <span className="text-[#888888] text-[10px] uppercase font-bold tracking-widest px-2 py-1 bg-[#F7F7F5] border border-[#E8E8E4] rounded">Achevé</span>
                        )}
                      </td>
                      <td className="py-6 px-8 text-right">
                        <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            ...
                            className="w-10 h-10 rounded-full bg-white border border-[#E8E8E4] flex items-center justify-center text-[#888888] hover:text-[#0A0A0A] hover:border-[#0A0A0A] transition-all shadow-sm"
                            title="Modifier"
                          >
                          ...
                          <button 
                            ...
                            className="w-10 h-10 rounded-full bg-white border border-[#E8E8E4] flex items-center justify-center text-[#888888] hover:text-red-600 hover:border-red-600 transition-all shadow-sm"
                            title="Supprimer"
                          >
